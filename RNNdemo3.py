...
test_point = 85000#使用的数据量大小（可调数 且必须大于seperate_point）
...
#print(weight_shape)# 10,2
#print(bias_shape)#2,
prediction = tf.matmul(last, weight) + bias
prediction_shape = prediction.get_shape()
print(prediction_shape)# ?,2
...
#saver.restore(sess, r"parameter.ckpt")
...
for i in range(epoch):
    ...
    for k in range(len(train_input)):
        inp, out = train_input[k:k + 1], train_output[k:k + 1]
        temp2 = sess.run(error, {data: inp, target: out})
        total_error1 += temp2
    total_error1 /= seperate_point
    total_error1 = math.sqrt(total_error1)

    for h in range(len(test_input)):
        inp, out = test_input[h:h + 1], test_output[h:h + 1]
        temp = sess.run(error, {data: inp, target: out})
        total_error += temp
    total_error /= (test_point - test_point_start)
...
#print(predict_result)
saver.save(sess, r"parameter.ckpt")
sess.close()
...
for cursor in range(16):
    ...
    #pylab.plot(test_output_single)# predict_result1是训练样本的检擦结果predict_result
    #pylab.plot(predict_result_single)#train_output1是训练样本的检查结果test_output1
...
for cursor in range(16):
    ...
    #pylab.plot(train_output_single)# predict_result1是训练样本的检擦结果predict_result
    #pylab.plot(predict_result1_single)#train_output1是训练样本的检查结果test_output1
...
csvfile = open(r'wind_short_result.csv', 'a')
...
writer.writerow([total_error])
csvfile.close()
csvfile = open(r'wind_long_result.csv', 'a')
writer = csv.writer(csvfile)
writer.writerow(['epoch', 'seperate_point', 'test_point_start', 'test_point', 'num_hidden', 'sequence_length',
                 'predict_length', 'batch_size'])
data = [(epoch, seperate_point, test_point_start, test_point, num_hidden, sequence_length, predict_length,
...
for i in range(len(predict_result)):
    writer.writerow(predict_result[i])
csvfile.close()
